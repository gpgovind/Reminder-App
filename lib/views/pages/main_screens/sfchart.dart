...
class _WaterProgressChartState extends State<WaterProgressChart> {
  ...
  void _loadData() async {
    ...
    final drinkBox = await Drinkbox.getdata();
            ...
            child: Column(
              ...
              children: [
                ...
                Padding(
                  ...
                  child: Text('Today goal : ${Watergoal.getdata().get(0)?.addgoal.toString()} ',
                   style: const TextStyle(fontSize: 15,fontWeight: FontWeight.w500),),
                  
                ),
                const SizedBox(height: 10),
                Text(
                    'water count : ${(_waterCount / _waterGoal * 100).toStringAsFixed(0)}%',
                     style: const TextStyle(fontSize: 15,fontWeight: FontWeight.w500),),
                    
                const SizedBox(height: 10),
                // Display the percentage of remaining water
                Text(
                    'remaining : ${((_waterGoal - _waterCount) / _waterGoal * 100).toStringAsFixed(0)}% ',
                    style: const TextStyle(fontSize: 15,fontWeight: FontWeight.w500),),
                const SizedBox(height: 10),
              ],
            ),
